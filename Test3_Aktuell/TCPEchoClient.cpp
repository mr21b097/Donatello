...
int main(int argc, char* argv[]) {
    ...
    unsigned int totalBytesRcvd;   // Gesamtzahl der empfangenen Bytes
    int bytesRcvd;                 // Anzahl der gelesenen Bytes in einem recv()
    ...
    echoServPort = (argc == 4) ? static_cast<unsigned short>(std::stoi(argv[3])) : 7;
    ...
    if (send(sock, echoString.c_str(), static_cast<int>(echoStringLen), 0) != static_cast<int>(echoStringLen)) {
